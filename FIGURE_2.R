...
tiff("plots/FINAL_FIGURE_2.tiff",width=270,height=240,units="mm",res=300, compression="lzw")

par(mar=c(6,5,2.9,2), mfrow=c(2,2))
...
mtext("HRP2 concentration (pg/ml)", 1, line=6)
mtext("Sensitivity (%)", 2, line=3)

legend("topleft", c("HS-RDT","co-RDT"), col = cols, pch=15, pt.cex = 1.6, bty="n", cex=1.1)
mtext("A", side = 3, adj = -0.18, line=1, cex=1.2, font=2)
...
legend("topleft", c("HS-RDT, Uganda", "HS-RDT, Myanmar", "co-RDT, Uganda", "co-RDT, Myanmar"),
       col=rep(cols2, each = 2), lwd=3, lty=c(1,2,1,2), bty="n")

mtext("C", side = 3, adj = -0.18, line=1, cex=1.2, font=2)

##########################################################################################################################

plot(log10(xseq3), pred_ci_pcr_1[,1], type="l", xaxt="n", las=1, 
     xlab="", main = "Probability of positivity by parasite density",
     ylab = "", lwd=3, col=cols2[1], ylim=c(0,1))
abline(h = seq(0,1, by=0.1), col="grey80", lty=3)
abline(v= seq(-1, 8, by=1), col="grey80", lty=3)

mtext(expression(paste("Parasite density (parasites per ",mu,"l)")), 1, line=3)
mtext("Probabilty of HS-RDT or co-RDT positivity", 2, line=3)
...
legend("bottomright", c("HS-RDT, Uganda", "HS-RDT, Myanmar", "co-RDT, Uganda", "co-RDT, Myanmar"),
       col=rep(cols2, each = 2), lwd=3, lty=c(1,2,1,2), bty="n")
mtext("D", side = 3, adj = -0.18, line=1, cex=1.2, font=2)
