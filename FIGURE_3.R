...
tiff("plots/FIG3_FINAL_1.tiff", width = 230,height=160,units="mm",res=300, compression="lzw")

par(mar=c(4,4,2.5,1), mfrow=c(1,2),oma=c(8.2,0,0,0), xpd=FALSE)
plot(bc_pcr$mean*100, bc_urdt$mean*100, pch=21, bg = "white", cex=1.5,
...
points(bc_pcr$mean*100, bc_urdt$mean*100, pch=21, bg = adjustcolor(bc_pcr$col, 0.8), cex=1.5)

lines(pcr_prev*100, rdt_prev*100, col="orange", lwd=2, lty=5)

mtext("A", side = 3, adj=-0.1, font=2, cex=1.3)
...
mtext("B", side = 3, adj=-0.1, font=2, cex=1.3)

par(xpd=NA)

bc_pcr_leg = arrange(bc_pcr, mean)

legend(-10, -2.0, paste0(bc_pcr_leg$Study)[1:9], pch=21, 
       pt.bg = adjustcolor(bc_pcr_leg$col, 0.8)[1:9], pt.cex = 1.6, bty="n", cex=0.78)
legend(-4, -2.0, paste0(bc_pcr_leg$Study)[10:18], pch=21, 
       pt.bg = adjustcolor(bc_pcr_leg$col, 0.8)[10:18], pt.cex = 1.6, bty="n", cex=0.78)

legend(2, -3.6, "Fitted line from Wu\net al. PCR vs. co-RDT\nprevalence relationship", cex=0.78, col="orange", lwd=2, lty=2, bty="n")
